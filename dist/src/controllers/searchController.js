...
exports.getSearchStats = exports.getSearchSuggestions = exports.globalSearch = void 0;
const client_1 = require("@prisma/client");
const logger_1 = require("../utils/logger");
const prisma = new client_1.PrismaClient();
const globalSearch = async (req, res) => {
    try {
        const { query, modules = [], limit = 20, offset = 0, includeInactive = false } = req.query;
...
// Search Products
async function searchProducts(query, isAdmin, includeInactive) {
    const whereClause = {
        OR: [
...
// Search Shops
async function searchShops(query, userPublicId, isAdmin, includeInactive) {
    let whereClause = {
        OR: [
            { name: { contains: query, mode: 'insensitive' } },
...
// Search Employees
async function searchEmployees(query, userPublicId, isAdmin, includeInactive) {
    let whereClause = {
...
// Search Inventory
async function searchInventory(query, userPublicId, isAdmin, includeInactive) {
    let whereClause = {
        OR: [
...
// Search Billing/Invoices
async function searchBilling(query, userPublicId, isAdmin, includeInactive) {
    let whereClause = {
        OR: [
...
// Search Restock Requests
async function searchRestockRequests(query, userPublicId, isAdmin, includeInactive) {
    let whereClause = {
        OR: [
...
// Search Categories
async function searchCategories(query, includeInactive) {
    const whereClause = {
        name: { contains: query, mode: 'insensitive' }
    };
    if (!includeInactive) {
...
// Search Flavors
async function searchFlavors(query, includeInactive) {
    const whereClause = {
        name: { contains: query, mode: 'insensitive' }
    };
    if (!includeInactive) {
...
function calculateRelevanceScore(query, fields) {
...
const getSearchSuggestions = async (req, res) => {
    try {
        ...
        logger_1.logger.error("Error getting search suggestions:", error);
