...
class PaymentController {
    ...
    static async uploadReceipt(req, res) {
        try {
            ...
            if (!restockRequest) {
                ...
                await (0, fileUploadService_1.deleteFile)(req.file.filename);
            ...
            logger_1.logger.error('Error uploading receipt:', error);
    ...
    static async verifyPayment(req, res) {
        try {
            ...
            if (verified && restockRequest.finalAmount) {
                await client_1.prisma.shopFinancials.upsert({
    ...
    static async getReceipt(req, res) {
        try {
            ...
            if (!(0, fileUploadService_1.fileExists)(filename)) {
                res.status(404).json({ error: 'File not found' });
                return;
            }
            const filePath = (0, fileUploadService_1.getFilePath)(filename);
            const fileStats = (0, fileUploadService_1.getFileStats)(filename);
            if (!fileStats) {
            ...
            res.download(filePath, filename, (err) => {
                if (err) {
                    logger_1.logger.error('Error downloading file:', err);
    ...
    static async getPaymentQueue(req, res) {
        try {
            const userId = req.user?.publicId;
            if (!userId) {
                res.status(401).json({ error: 'Unauthorized' });
            ...
            const paymentQueue = await client_1.prisma.restockRequest.findMany({
                where: {
                    paymentStatus: 'pending_verification',
            ...
            logger_1.logger.error('Error getting payment queue:', error);
            res.status(500).json({ error: 'Failed to get payment queue' });
        }
    }
    // Get shop financials
    static async getShopFinancials(req, res) {
        try {
            ...
            const financials = await client_1.prisma.shopFinancials.findUnique({
                where: { shopId },
                include: {
                    shop: {
                        select: {
            ...
            if (!financials) {
                ...
                const newFinancials = await client_1.prisma.shopFinancials.create({
                    data: {
                        shopId,
                        totalRevenue: 0,
                        totalExpenses: 0,
            ...
            logger_1.logger.error('Error getting shop financials:', error);
