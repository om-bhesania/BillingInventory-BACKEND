...
class StockAdjustmentController {
    ...
    static async createStockAdjustment(req, res) {
        try {
            ...
            const shop = await client_1.prisma.shop.findFirst({
                where: {
                    id: shopId,
                    managerId: userId
                }
            ...
            const stockAdjustment = await client_1.prisma.stockAdjustmentRequest.create({
                data: {
                    shopId,
                    productId,
                    currentStock,
            ...
            logger_1.logger.error('Error creating stock adjustment request:', error);
    ...
    static async getStockAdjustments(req, res) {
        try {
            ...
            const stockAdjustments = await client_1.prisma.stockAdjustmentRequest.findMany({
                where: whereClause,
                include: {
                    shop: {
                        select: {
    ...
    static async updateStockAdjustmentStatus(req, res) {
        try {
            ...
            const stockAdjustment = await client_1.prisma.stockAdjustmentRequest.findUnique({
            ...
            const updatedAdjustment = await client_1.prisma.stockAdjustmentRequest.update({
                where: { id: adjustmentId },
                data: {
                    status,
                    approvedBy: userId,
            ...
            if (status === 'approved') {
                ...
                await client_1.prisma.shopInventory.upsert({
                    where: {
                        shopId_productId: {
                ...
                await client_1.prisma.stockTransaction.create({
                    data: {
                        shopId: stockAdjustment.shopId,
                        productId: stockAdjustment.productId,
            ...
            if (stockAdjustment.shop.managerId) {
                ...
                (0, NotificationsController_1.emitUserNotification)(stockAdjustment.shop.managerId, {
            ...
            logger_1.logger.error('Error updating stock adjustment status:', error);
            res.status(500).json({ error: 'Failed to update stock adjustment status' });
        }
    }
    // Get default adjustment reasons
    static async getDefaultReasons(req, res) {
        try {
            ...
            logger_1.logger.error('Error fetching default reasons:', error);
            res.status(500).json({ error: 'Failed to fetch default reasons' });
        }
    }
    // Get stock adjustment details
    static async getStockAdjustmentDetails(req, res) {
        try {
            const { adjustmentId } = req.params;
            const userId = req.user?.publicId;
            if (!userId) {
            ...
            logger_1.logger.error('Error fetching stock adjustment details:', error);
