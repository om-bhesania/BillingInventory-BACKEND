...
exports.getUserShopIds = exports.removeShopIdFromUser = exports.addShopIdToUser = void 0;
 ...
 */
const addShopIdToUser = async (userPublicId, shopId) => {
    try {
        ...
        if (!user) {
            logger_1.logger.error(`User not found with publicId: ${userPublicId}`);
            return;
        }
        ...
        if (!shop) {
            logger_1.logger.error(`Shop not found with id: ${shopId}`);
            return;
        }
        // Check if shop is already managed by this user
        if (shop.managerId === userPublicId) {
            logger_1.logger.info(`Shop ID ${shopId} is already managed by user ${userPublicId}`);
            return;
        }
        // Update shop to set this user as manager
        await client_1.prisma.shop.update({
            where: { id: shopId },
            data: { managerId: userPublicId }
        });
        logger_1.logger.info(`Added shop ID ${shopId} to user ${userPublicId} as manager`);
        ...
        logger_1.logger.error(`Error adding shop ID ${shopId} to user ${userPublicId}:`, error);
        throw error;
    }
};
exports.addShopIdToUser = addShopIdToUser;
/**
 * Remove a shop ID from a user's managed shops (temporary implementation)
 * @param userPublicId - The user's public ID
 * @param shopId - The shop ID to remove
 */
const removeShopIdFromUser = async (userPublicId, shopId) => {
    try {
        ...
        if (shop.managerId !== userPublicId) {
            logger_1.logger.info(`Shop ID ${shopId} is not managed by user ${userPublicId}`);
            return;
        }
        ...
        logger_1.logger.info(`Removed shop ID ${shopId} from user ${userPublicId}`);
    }
    catch (error) {
        logger_1.logger.error(`Error removing shop ID ${shopId} from user ${userPublicId}:`, error);
        throw error;
    }
};
exports.removeShopIdFromUser = removeShopIdFromUser;
/**
 * Get all shop IDs for a user (temporary implementation)
 * @param userPublicId - The user's public ID
 * @returns Array of shop IDs
 */
const getUserShopIds = async (userPublicId) => {
    try {
        ...
        logger_1.logger.error(`Error getting shop IDs for user ${userPublicId}:`, error);
        return [];
    }
};
exports.getUserShopIds = getUserShopIds;
/**
