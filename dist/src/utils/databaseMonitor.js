...
class DatabaseMonitor {
     ...
     */
    static async getTablesWithHighSeqScans() {
        try {
            const result = await prisma.$queryRaw `
        SELECT 
          schemaname,
    ...
    static async getHealthReport() {
        try {
            const [databaseSize, indexStats, tableStats, unusedIndexes, highSeqScanTables, slowQueries] = await Promise.allSettled([
     ...
     */
    static generateRecommendations(healthReport) {
        const recommendations = [];
        // Check for unused indexes
        if (healthReport.unusedIndexes.length > 0) {
            recommendations.push(`Consider removing ${healthReport.unusedIndexes.length} unused indexes to improve write performance: ${healthReport.unusedIndexes.map(i => i.indexName).join(', ')}`);
        }
        // Check for tables with high sequential scans
        if (healthReport.highSeqScanTables.length > 0) {
            recommendations.push(`Consider adding indexes for tables with high sequential scans: ${healthReport.highSeqScanTables.map(t => t.tableName).join(', ')}`);
        }
        ...
        if (healthReport.slowQueries.length > 0) {
            ...
            if (avgSlowQueryTime > 1000) {
                recommendations.push(`Found ${healthReport.slowQueries.length} slow queries with average execution time of ${avgSlowQueryTime.toFixed(2)}ms. Consider optimizing these queries.`);
            }
        }
        // Check database size
        const dbSizeGB = parseFloat(healthReport.databaseSize.size.replace(/[^\d.]/g, ''));
        if (dbSizeGB > 1) {
            recommendations.push(`Database size is ${healthReport.databaseSize.size}. Consider archiving old data or implementing data partitioning.`);
        }
