...
export class DashboardController {
  ...
  static async getDashboardMetrics(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const fromDate = (req as any).query.from ? new Date((req as any).query.from as string) : null;
      const toDate = (req as any).query.to ? new Date((req as any).query.to as string) : null;
  ...
  static async getRecentActivities(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const whereClause: any = isAdmin ? {} : { shopId: { in: userShopIds } };
      let auditLogs: any[] = [];
      try {
      ...
      let activities: any[] = [];
      if (isAdmin) activities = await DashboardController.getAllRecentActivities();
      else activities = userShopIds.length === 0 ? [] : await DashboardController.getShopRecentActivities(userShopIds);
  ...
  private static async getTotalShops(startDate?: Date, endDate?: Date) {
  ...
  private static async getTotalProducts(startDate?: Date, endDate?: Date) {
  ...
  private static async getTotalCategories(startDate?: Date, endDate?: Date) {
  ...
  private static async getPendingRestockRequests(shopIds?: string[]) {
  ...
  private static async getShopPerformance(startDate?: Date, endDate?: Date) {
  ...
  // Shop Owner methods
  private static async getShopRevenue(shopIds: string[], startDate?: Date, endDate?: Date) {
    ...
    const whereClause: any = { shopId: { in: shopIds } };
  ...
  private static async getTopSellingProducts(shopIds: string[], startDate?: Date, endDate?: Date) {
  ...
  private static async getRestockExpenses(shopIds: string[], startDate?: Date, endDate?: Date) {
  ...
  private static async getTopSellingCategory(shopIds?: string[], startDate?: Date, endDate?: Date) {
    if (shopIds && shopIds.length > 0) {
      ...
      billings.forEach(b => {
        if (Array.isArray(b.items)) {
          (b.items as any[]).forEach((it: any) => {
            if (it.productId && it.quantity) productIds.add(it.productId);
          });
        }
      });
  ...
  private static async getTopSellingFlavor(shopIds?: string[], startDate?: Date, endDate?: Date) {
    if (shopIds && shopIds.length > 0) {
      // For Shop Owner: Use billing data (existing logic)
      const billings = await prisma.billing.findMany({ where: { shopId: { in: shopIds } }, select: { items: true } });
      ...
      const productIdToFlavor: Record<string, string> = {};
  ...
  private static async getCategoryBreakdown(shopIds?: string[], startDate?: Date, endDate?: Date) {
    if (shopIds && shopIds.length > 0) {
      // For Shop Owner: Use billing data (existing logic)
      const billings = await prisma.billing.findMany({ where: { shopId: { in: shopIds } }, select: { items: true } });
      const categoryCount: Record<string, number> = {};
      const productIds: Set<string> = new Set();
      billings.forEach(b => {
        if (Array.isArray(b.items)) (b.items as any[]).forEach((it: any) => { if (it.productId && it.quantity) productIds.add(it.productId); });
      });
      const products = await prisma.product.findMany({ where: { id: { in: Array.from(productIds) } }, include: { category: true } });
      const productIdToCategory: Record<string, string> = {};
      products.forEach(p => { productIdToCategory[p.id] = p.category?.name || "Unknown"; });
      billings.forEach(b => {
        if (Array.isArray(b.items)) (b.items as any[]).forEach((it: any) => {
          if (it.productId && it.quantity) {
            const cat = productIdToCategory[it.productId] || "Unknown";
            categoryCount[cat] = (categoryCount[cat] || 0) + it.quantity;
          }
  ...
  private static async getFlavorBreakdown(shopIds?: string[], startDate?: Date, endDate?: Date) {
    if (shopIds && shopIds.length > 0) {
      // For Shop Owner: Use billing data (existing logic)
      const billings = await prisma.billing.findMany({ where: { shopId: { in: shopIds } }, select: { items: true } });
      const flavorCount: Record<string, number> = {};
  ...
  private static async getSalesTrend(shopIds?: string[], startDate?: Date, endDate?: Date) {
  ...
  private static async getCurrentStockLevels(shopIds: string[]) {
    ...
    const lowStockItems = stockLevels.filter(item => 
      item.currentStock <= (item.product.minStockLevel || 10)
    );
  ...
  private static async getShopNotifications(shopIds: string[]) {
  ...
  // Activity methods
  private static async getAllRecentActivities() {
    const activities = [];
  ...
  private static async getShopRecentActivities(shopIds: string[]) {
    const activities = [];
