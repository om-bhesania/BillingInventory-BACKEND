...
export const updateEmployeeStatus = async (req: Request, res: Response): Promise<any> => {
  try {
    const { id } = req.params;
...
export const getEmployeeRoles = async (req: Request, res: Response): Promise<any> => {
  try {
...
export const getAvailableShops = async (req: Request, res: Response): Promise<any> => {
  try {
    const userPublicId = (req as any).user?.publicId;
    if (!userPublicId) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    logger.error("Error fetching available shops:", error);
