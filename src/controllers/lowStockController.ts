...
export class LowStockController {
  ...
  static async getLowStockAlerts(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const { page = 1, limit = 20, category, flavor, shopId } = (req as any).query;
      ...
      if (shopId && shopId !== 'all') {
        if (isAdmin || userShopIds.includes(shopId as string)) {
  ...
  static async getLowStockStats(req: AuthenticatedRequest, res: Response) {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ error: 'User not authenticated' });
  ...
  static async getLowStockFilters(req: AuthenticatedRequest, res: Response) {
    try {
      const user = req.user;
      if (!user) {
        return res.status(401).json({ error: 'User not authenticated' });
  ...
  // Admin methods
  private static async getAllLowStockItems(whereClause: any, limit: number, offset: number) {
  ...
  private static async getLowStockItemsCount(whereClause: any) {
  ...
  private static async getAllLowStockStats() {
  ...
  private static async getAllLowStockFilters() {
  ...
  private static async getShopLowStockFilters(shopIds: string[]) {
  ...
  private static calculateUrgency(currentStock: number, minStockLevel: number): 'critical' | 'warning' | 'low' {
