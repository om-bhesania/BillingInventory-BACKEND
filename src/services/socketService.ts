...
interface SocketService {
  ...
  broadcastInventoryUpdate(shopId: string, data: any): void;
  broadcastRestockRequestUpdate(data: any): void;
  broadcastBillingUpdate(shopId: string, data: any): void;
  broadcastProductUpdate(data: any): void;
  broadcastShopUpdate(data: any): void;
  broadcastDashboardUpdate(data: any): void;
  broadcastLowStockAlert(shopId: string, data: any): void;
...
class SocketServiceImpl implements SocketService {
  ...
  private async authenticateSocket(socket: AuthenticatedSocket, next: (err?: Error) => void) {
    try {
      ...
      if (!user) {
        logger.warn('Socket authentication failed: User not found', { publicId: decoded.publicId });
  ...
  private handleConnection(socket: AuthenticatedSocket) {
    ...
    logger.info(`User connected: ${userPublicId} (${userRole})`);
    ...
    socket.on('join:room', (data: { room: string }) => {
      ...
      logger.info(`User ${userPublicId} joined room ${data.room}`);
    });

    // Handle leaving chat rooms
    socket.on('leave:room', (data: { room: string }) => {
      socket.leave(data.room);
      logger.info(`User ${userPublicId} left room ${data.room}`);
    });

    // Handle message read receipts
    socket.on('chat:message:read', async (data: { messageId: string }) => {
      try {
      ...
      if (data.room.startsWith('chat-request-') && userRole === 'Admin') {
        ...
        try {
          ...
          if (chatRequest && chatRequest.status === 'pending') {
            ...
            logger.info(`Chat request ${chatRequestId} auto-assigned to admin ${userPublicId}`);
          }
        } catch (error) {
          logger.error('Error auto-assigning chat request:', error);
        }
      }
    ...
    socket.on('live:subscribe', (data: { type: string }) => {
      ...
      logger.info(`User ${userPublicId} subscribed to live data: ${data.type}`);
    });

    socket.on('live:unsubscribe', (data: { type: string }) => {
      socket.leave(`live:${data.type}`);
      logger.info(`User ${userPublicId} unsubscribed from live data: ${data.type}`);
    });
    ...
    socket.on('dashboard:dateRange:change', async (data: { dateRange: any, comparisonType?: string, preferences?: any }) => {
      try {
        logger.info(`Dashboard date range change requested by ${userPublicId}:`, data);
        ...
        logger.info(`Dashboard date range updated for ${userPublicId}`);
      } catch (error) {
        logger.error('Error handling dashboard date range change:', error);
        socket.emit('dashboard:dateRange:error', { 
    ...
    socket.on('dashboard:preferences:save', async (data: { preferences: any }) => {
      try {
        await this.saveUserPreferences(userPublicId, data.preferences);
        ...
        logger.info(`User preferences saved for ${userPublicId}`);
      } catch (error) {
    ...
    socket.on('dashboard:preferences:load', async () => {
      try {
        ...
        logger.info(`User preferences loaded for ${userPublicId}`);
      } catch (error) {
    ...
    socket.on('dashboard:subscribe:live', (data: { type: string }) => {
      ...
      logger.info(`User ${userPublicId} subscribed to live dashboard data: ${data.type}`);
    ...
    socket.on('dashboard:unsubscribe:live', (data: { type: string }) => {
      ...
      logger.info(`User ${userPublicId} unsubscribed from live dashboard data: ${data.type}`);
    });

    socket.on('dashboard:subscribe:insights', () => {
      socket.join('dashboard:insights');
      logger.info(`User ${userPublicId} subscribed to dashboard insights`);
    ...
    socket.on('dashboard:unsubscribe:insights', () => {
      ...
      logger.info(`User ${userPublicId} unsubscribed from dashboard insights`);
    });

    // Handle disconnection
    socket.on('disconnect', (reason) => {
      this.connectedUsers.delete(userId);
      logger.info(`User disconnected: ${userPublicId} (${reason})`);
  ...
  private async handleNotificationRead(userPublicId: string, notificationId: string) {
    ...
    logger.info(`Notification ${notificationId} marked as read by user ${userPublicId}`);
  }

  private async joinUserToShopRooms(socket: AuthenticatedSocket, userPublicId: string, userRole: string) {
    try {
      ...
      logger.error('Error joining user to shop rooms:', error);
    }
  }

  private async handleChatMessage(userPublicId: string, message: string, room: string) {
    ...
    if (!user) {
      logger.error(`User not found: ${userPublicId}`);
      return;
    }
    ...
    if (!chatRequest) {
      logger.error(`Chat request not found: ${chatRequestId}`);
      return;
    }
    ...
    logger.info(`Chat message sent by ${userPublicId} to room ${room}`);
  }

  private async handleMessageRead(userPublicId: string, messageId: string) {
    ...
    logger.info(`Message ${messageId} marked as read by ${userPublicId}`);
  }
  ...
  private validateDateRange(dateRange: any): { isValid: boolean; error?: string } {
  ...
  private calculateComparisonPeriod(dateRange: any, comparisonType?: string) {
  ...
  private async generateRealMetrics(dateRange: any, comparisonPeriod: any, userPublicId: string, userRole: string) {
    try {
      ...
      logger.error('Error generating real metrics:', error);
      // Fallback to mock data
      return this.generateMockMetrics(dateRange, comparisonPeriod);
    }
  }

  private async getPeriodMetrics(fromDate: Date, toDate: Date, shopFilter: any) {
  ...
  private async generateMockMetrics(dateRange: any, comparisonPeriod: any) {
  ...
  private async saveUserPreferences(userPublicId: string, preferences: any) {
    try {
  ...
  private async loadUserPreferences(userPublicId: string) {
    try {
      const user = await prisma.user.findUnique({
        where: { publicId: userPublicId },
        select: { 
  ...
  private startLiveDataBroadcast(socket: AuthenticatedSocket, type: string, userPublicId: string) {
    ...
    (socket as any).liveDataInterval = interval;
  }

  private startInsightsBroadcast(socket: AuthenticatedSocket, userPublicId: string) {
    const insights = [
      {
        id: '1',
        type: 'growth',
    ...
    // Store interval reference for cleanup
    (socket as any).insightsInterval = interval;
  }
}
